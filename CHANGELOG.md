...
## 2026-09-24 — Scene text box label reads "Contents"
...
- A project from an unfinished import shows a note on its page, also after the import is purged.
...
- List searches and the revision label filter now match `%` and `_` as plain characters, not as wildcards.
...
- The revision pages read the entities they cover from the same list the rest of the
  history feature uses.
