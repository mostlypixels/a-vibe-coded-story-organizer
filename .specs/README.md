...
.specs/
  ...
  shelved/<name>/              # a draft parked for a while (flat — no date either)
...
The five status folders are exactly the five `status:` values (see the table below). Four
are lifecycle stages; `shelved` is a siding off stage 1 (see *Shelving*). A subfolder (or month bucket) only exists when at least one feature is at that
...
Because a feature can sit under any status, always locate one by name with the **three
globs** `.specs/draft/<name>/`, `.specs/shelved/<name>/` and `.specs/*/*/<name>/` (never a
hard-coded status or bucket). `App\Support\SpecTree::locate()` does this for PHP callers.
...
Each move (`mp-expand-spec` → `expanded/`, `mp-plan-tasks` → `planned/`, `ship-plan` →
...
```
.specs/<status>/<YYYY-MM>/<name>/    (drafts and shelved: .specs/<status>/<name>/)
  spec.md              # the short source spec you write by hand (the input)
  expanded/            # detailed design docs generated by /mp-expand-spec
    overview.md  data-model.md  architecture.md  ui.md  testing.md  open-questions.md  (+ feature-specific)
  plan/                # ordered task files generated by /mp-plan-tasks
...
|-------|---------|-------|--------|----------------------|
| 1. Draft | `/mp-draft-spec <name>` *(or by hand)* | your request | `.specs/draft/<name>/spec.md` | `status: draft` → `draft/<name>/` |
| — Shelve | `php artisan spec:shelve <name>` | a draft | nothing new | `status: shelved` → `shelved/<name>/` |
| 2. Expand | `/mp-expand-spec <name>` | `spec.md` | `expanded/` | `status: expanded` + `expanded: <date>` → `expanded/<YYYY-MM>/<name>/` |
| 3. Plan | `/mp-plan-tasks <name>` | `expanded/` | `plan/` + `resolution-log.md` | `status: planned` + `planned: <date>` → `planned/<YYYY-MM>/<name>/` |
...
A draft that nobody will pick up for months buries the ones that are queued. Move it to
`.specs/shelved/<name>/` instead of deleting it or marking it low-priority in prose that
no listing sorts by. The shelf is a place, not a stage — no date stamp, no month bucket,
and no work is lost.

```
php artisan spec:shelve <name> [--reason="why it waits"]   # draft/<name>/ → shelved/<name>/
php artisan spec:unshelve <name>                           # shelved/<name>/ → draft/<name>/
```

- Both commands re-stamp `status:` in the frontmatter, so the folder and the stamp always
  agree. `App\Services\SpecShelf` holds the rules.
- `--reason` writes a `> [!NOTE] **Shelved.** …` callout under the title. `spec:unshelve`
  removes it again.
- Only a draft shelves. An expanded or planned spec carries design work and a plan; park
  one of those and you have a harder problem than this feature solves.
- The pipeline skills ignore `shelved/`. `/mp-expand-spec <name>` on a shelved feature
  finds nothing — unshelve it first.
- The name still has to be unique across the whole tree. A shelved spec holds its name.

## Starting a new feature

The simplest path is to ask for the draft — the `mp-draft-spec` skill files it correctly:

```
/mp-draft-spec <name>               # writes .specs/draft/<name>/spec.md with status: draft
/mp-expand-spec <name>         # moves the folder to expanded/<YYYY-MM>/ when done
/mp-plan-tasks <name>               # moves the folder to planned/<YYYY-MM>/ when done
...
immediately. `/mp-draft-spec` handles this: it suffixes (`<name>-YYYY-MM-DD`) or picks a
