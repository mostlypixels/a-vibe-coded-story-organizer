...
>
    <div x-show="selectedValues.length" class="mb-2 flex flex-wrap gap-2">
        <template x-for="value in selectedValues" :key="value">
            <span class="inline-flex items-center gap-1 rounded-full bg-accent-surface px-2 py-0.5 text-xs font-medium text-accent-content">
                <span x-text="labelFor(value)"></span>
                <span class="text-accent-content/70" x-show="sublabelFor(value)" x-text="sublabelFor(value)"></span>
                <button type="button" @click="remove(value)" class="ml-0.5 text-accent-content/70 hover:text-accent-content" :aria-label="'{{ __('Remove') }} ' + labelFor(value)">&times;</button>
    ...
    <div class="relative">
        ...
        <ul x-show="results.length || canAddFreeText" x-transition style="display: none;" class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md border border-border bg-surface-overlay py-1 shadow-lg">
            <template x-for="opt in results" :key="opt.value">
                <li>
                    <button type="button" @click="add(opt.value)" class="flex w-full items-center justify-between gap-2 px-3 py-2 text-left text-sm hover:bg-neutral">
                        <span class="font-medium text-content" x-text="opt.label"></span>
                        <span class="text-content-muted" x-show="opt.sublabel" x-text="opt.sublabel"></span>
                    </button>
                </li>
            </template>

            <li x-show="canAddFreeText">
                <button type="button" @click="add(query.trim())" class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-neutral">
                    <span class="text-content-muted">{{ __('Add') }}</span>
                    <span class="font-medium text-content" x-text="'“' + query.trim() + '”'"></span>
                </button>
            </li>
        </ul>

        <p x-show="query.trim() !== '' && results.length === 0 && ! canAddFreeText" style="display: none;" class="mt-1 text-sm text-content-muted">{{ __('No matches.') }}</p>
    </div>
</div>
