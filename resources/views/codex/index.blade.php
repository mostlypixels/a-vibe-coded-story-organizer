    ...
    <div class="space-y-6">
            <div class="flex items-center justify-between gap-4">
                <form method="GET" class="flex items-center gap-2">
                    ...
                    <select name="tag" class="text-sm border-gray-300 focus:border-ocean-500 focus:ring-ocean-500 rounded-md shadow-2xs">
                    ...
                    <x-table-row :striped="$loop->even">
                        <td class="px-4 py-3">
                            <a href="{{ route('codex.edit', $entry) }}">
                                @if ($entry->cover)
                                    <img src="{{ $entry->cover->url() }}" alt="{{ $entry->name }}" class="h-10 w-10 rounded-xs object-cover border border-gray-200">
                                @else
