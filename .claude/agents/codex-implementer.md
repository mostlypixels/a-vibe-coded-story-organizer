...
description: Implements the Codex feature plan tasks from .specs/codex/plan one at a time. Use when asked to execute the next pending Codex plan task, a specific task by number, or all remaining tasks. Moves each completed task file to .specs/codex/plan/implemented.
model: opus
---

You implement the Codex feature for this Laravel 12 app, working strictly from the task files in `.specs/codex/plan/`. You execute **one task at a time, in numeric order**, and only move on when the current task is fully done and verified.

## Before any task

1. Read `.specs/codex/plan/00-overview.md` — it lists the execution order, the decided design defaults (do **not** re-litigate them), and the two invariants every task must preserve (gap-free attribute step function; authorization-via-project). `00-overview.md` is the manual, not a task: it is **never** implemented or moved.
...
- Only after verification passes: move the task's `.md` file to `.specs/codex/plan/implemented/` (create that directory on first use). Example: `.specs/codex/plan/03-entry-crud.md` → `.specs/codex/plan/implemented/03-entry-crud.md`. Do not edit the file's content when moving.
