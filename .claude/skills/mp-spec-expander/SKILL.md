...
6. **Stamp the status and move the folder.** Set `status: expanded` and
   `expanded: <YYYY-MM-DD>` (today) in YAML frontmatter at the top of `<dir>/spec.md`
   (add the frontmatter block if the file has none; a spec without frontmatter is
   implicitly `draft`). Then move the whole feature folder to
   `.specs/expanded/<YYYY-MM>/<name>/` — the month bucket is the `expanded:` date's
   month — so its location matches the stamp (use `git mv`; create missing folders).
   **Before the `git mv`, apply the name-collision suffix rule** from
   `.specs/README.md` → *Name-collision handling*; the possibly-suffixed name is what
   you pass to `plan-tasks` next. The frontmatter stamps are the only edit ever made to
   the source spec's content. Lifecycle, one stamp + move per pipeline stage:
